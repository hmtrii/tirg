    ...
    def __init__(self, path, split='train', transform=None):
        ...
        for filename in label_files:
            ...
            for line in lines:
                ...
                captions = line[3].split(';')
                if len(captions) <= 2:
                  captions[0] = captions[0].replace('METADATA: ', '')                
                else:
                  if captions[1][0:2] == 'W ':
                    del captions[1]
                  captions = captions[1:]
                captions = [c for c in captions if c != 'null']
                captions[-1] = captions[-1].replace('\n', '')
                  # captions = [t.replace(' ', '') for t in captions]
                  # captions = ' '.join([c for c in captions])
                  # if len(captions) == 1:
                  #   continue
                img = {
                    'file_path': line[1],
